...
// Skill card configuration (scores are fetched dynamically)
const SKILL_CONFIG = [
  {
...
function Dashboard() {
  ...
  // Dashboard stats (dynamic from Supabase)
  ...
  const [streakData, setStreakData] = useState<{ count: number; days: boolean[] }>({
    count: 0,
  ...
  // Streak details (dynamic)
  const streakDays = DAY_LABELS.map((day, i) => ({
    day,
    active: streakData.days[i] ?? false,
  }));
  ...
  return (
    <AppShell>
      <div className="max-w-6xl mx-auto px-4 py-8 animate-fade-in md:px-6">
          ...
          <div className="lg:col-span-2 rounded-3xl border border-border/80 bg-card p-6 md:p-8 hover-glow shadow-xl relative overflow-hidden flex flex-col justify-between min-h-[220px]">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground leading-relaxed max-w-xl mb-6">
                Practice writing and speaking with instant grammar checking, vocabulary
                enhancements, coherence evaluation, and automated band scoring.
              </p>
            </div>
            <div className="flex flex-wrap gap-4 mt-auto">
              <Button
                className="rounded-xl h-11 shadow-lg shadow-accent/20 hover:scale-[1.02] transition-transform"
                asChild
              >
                <Link to="/writing">
          ...
          <div className="rounded-3xl border border-border/80 bg-slate-900 text-slate-100 p-6 flex flex-col justify-between shadow-xl relative overflow-hidden hover-lift min-h-[220px]">
            ...
            <div>
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs bg-white/10 text-orange-300 font-bold px-2 py-0.5 rounded-full">
                  Level Active
                </span>
              </div>
              <div className="mt-4 flex items-baseline gap-2">
                <span className="text-6xl font-black font-heading tracking-tighter text-transparent bg-clip-text bg-gradient-to-tr from-orange-500 to-amber-400">
                  {streakData.count}
                </span>
              ...
              <p className="text-xs text-slate-350 mt-1.5">
                {streakData.count > 0
                  ? "You are doing great! Keep practicing daily to maintain it."
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:col-span-3">
            {SKILL_CONFIG.map((s) => {
              ...
              return (
                <div
                  key={s.name}
                  className="rounded-2xl border border-border/80 bg-card p-5 hover-lift hover-glow shadow-sm flex flex-col justify-between"
                >
                  <div>
                    <div className="flex items-center justify-between mb-4">
                      <div
                        className={`size-10 rounded-xl bg-gradient-to-tr ${s.gradient} text-white flex items-center justify-center shadow-md`}
                      >
                    ...
                    <div className="flex items-center gap-4 my-2">
                      {statsLoading ? (
                        <div className="relative size-14 flex items-center justify-center">
                          <div className="size-10 rounded-full border-2 border-muted animate-pulse" />
                        </div>
                      ) : score !== null ? (
                        <ProgressCircle percentage={score} strokeColor={s.stroke} />
                      ) : (
                        <EmptyProgressCircle />
                      )}
                      <div>
                        {statsLoading ? (
                          <>
                            <div className="h-5 w-12 bg-muted/60 rounded animate-pulse mb-1" />
                            <div className="h-3 w-20 bg-muted/40 rounded animate-pulse" />
                          </>
                        ) : (
                          <>
                            <div className="text-xl font-bold tracking-tight">
                              {score !== null ? `${score}%` : "No Record"}
                            </div>
                            <p className="text-[11px] text-muted-foreground">
                              {score !== null ? s.label : "Practice to track score"}
                            </p>
                          </>
                        )}
                      </div>
                  ...
                  {s.to ? (
                    <Button
                      ...
                      variant="ghost"
                      asChild
                      className="w-full mt-4 hover:bg-muted font-semibold group rounded-xl"
                    >
                      <Link to={s.to}>
                        Practice Skill{" "}
                ...
                <div className="relative min-h-[110px]">
                  {!isFlipped ? (
                    <div className="transition-all duration-300">
                      <div className="text-3xl font-extrabold tracking-tight mb-1 font-heading">
                        {wordData.word}
                      </div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        Click the button below to review the meaning and study examples.
                      </p>
                    </div>
                  ) : (
              ...
              <>
                <div>
                  ...
                  <p className="text-sm text-muted-foreground max-w-lg leading-relaxed">
                    Test your English vocabulary and core grammar skills with standard examiner
                    multiple-choice drills calibrated to your level.
                  </p>
                </div>
                <Button
                  onClick={startMiniTest}
                  className="w-fit mt-4 rounded-xl h-10 px-5 bg-accent hover:bg-accent/90 text-white shadow-md shadow-accent/15"
                >
                  Start Mini-Test
                </Button>
              </>
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground max-w-lg leading-relaxed">
                    Explore interactive flashcards, vocabulary match ups, fill-in-the-blanks, and
                    competitive word games calibrated to boost exam scores.
                  </p>
                </div>
              </div>
        ...
        <section className="mt-8">
          <h2 className="text-xl font-bold tracking-tight mb-4 font-heading">
            Pick your exam syllabus
          </h2>
